...
import { useEventTracker } from './EventTracker';
...
export function ShareButton({ title, url, articleSlug, className = '' }: ShareButtonProps) {
  const [isOpen, setIsOpen] = useState(false);
  const [copied, setCopied] = useState(false);
  const { trackShare } = useEventTracker();
  ...
  const handleCopyLink = async () => {
    try {
      ...
      setCopied(true);
      trackShare('copy-link', articleSlug);
  ...
  const handleSocialShare = (platform: string) => {
    ...
    switch (platform) {
      case 'twitter':
        shareUrl = `https://twitter.com/intent/tweet?text=${encodeURIComponent(shareData.text)}&url=${encodeURIComponent(url)}`;
        break;
      case 'linkedin':
